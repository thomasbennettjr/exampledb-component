...
public abstract class QueryManagerImpl implements QueryManager {
    ...
    public QueryManagerImpl(final I18nMessage i18n, final ExampleDBOutputConfiguration configuration)
    {
        this.i18n = i18n;
        this.configuration = configuration;
    }

    public ExampleDBOutputConfiguration getConfiguration() { return this.configuration; }

    public void truncate(String tableName, final ExampledbComponentService.DataSource dataSource) throws SQLException {
    ...
    @Override
    public List<Reject> execute(final List<Record> records, final ExampledbComponentService.DataSource dataSource) throws SQLException {
        if (records.isEmpty()) {
            return emptyList();
        }
        try (final Connection connection = dataSource.getConnection()) {
